...
const createPatient = async(first_name, last_name, email, hashPassword, status, gender, address, contact_number, data_of_birth, stepend, study_enrolled, notification,note) => {
    return new Promise((resolve, reject) => {
        db.beginTransaction((err) => {
            ...
            db.query(query, [first_name, last_name, email, hashPassword, status, gender, address, contact_number, data_of_birth, stepend, study_enrolled, notification] , (err, result) => {
                ...
                const selectQuery = 'SELECT p.*, n.note FROM patient AS p JOIN note as n ON p.patient_id = n.patient_id ';
                db.query(selectQuery, [patientId], (error, rows) => {
                    ...
                    const noteQuery = 'INSERT INTO Note (patient_id, note) VALUES (?, ?)';
...
// update patient
const updatePatient = (patient_id, first_name, last_name, email, hashPassword, status, gender, address, contact_number, data_of_birth, stepend, study_enrolled, notification) => {
    console.log("model", patient_id, first_name, last_name, email, hashPassword,status, gender, address, contact_number, data_of_birth, stepend, study_enrolled, notification);
    return new Promise((resolve, reject) => {
        const query = 'UPDATE patient SET first_name = ?, last_name = ?, email = ?, password = ?, status=?, gender=?, address = ?, contact_number = ?, date_of_birth = ?, stepend = ?, study_enrolled = ?, notification = ? WHERE patient_id = ?';
        db.query(query, [first_name, last_name, email, hashPassword, status, gender, address, contact_number, data_of_birth, stepend, study_enrolled, notification, patient_id], (err, result) => {
            if(err) {
                reject(err);
            } else {
                const selectQuery = 'SELECT * FROM patient WHERE patient_id = ?';
